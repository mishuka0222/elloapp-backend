#!/bin/bash
# if [ ! -n "$ELLOAPP_HOST" ]; then
#   echo ">>> Plase set environment variable ELLOAPP_HOST to your own server IP. <<<"
#   exit 1
# fi

export ELLOAPP_HOST=${ELLOAPP_HOST:-"0.0.0.0"}
export ETCD_URL=${ETCD_URL:-"etcd:2379"}
export REDIS_HOST=${REDIS_HOST:-"redis:6379"}
export KAFKA_HOST=${KAFKA_HOST:-"kafka:9092"}
export MYSQL_URI=${MYSQL_URI:-"elloapp:elloapp@tcp(mysql:3306)/elloapp?charset=utf8mb4"}
...
createConfigs() {
  ...
  for file in `ls $CONFIG_TEMPLATES_DIR`; do
    cat $CONFIG_TEMPLATES_DIR/$file \
      | sed 's#ListenOn: 127.0.0.1#ListenOn: '"$ELLOAPP_HOST"'#g' \
      | sed "s#127.0.0.1:2379#$ETCD_URL#g" \
      | sed "s#127.0.0.1:6379#$REDIS_HOST#g" \
      | sed "s#localhost:6379#$REDIS_HOST#g" \
      | sed "s#root:@tcp(127.0.0.1:3306)/elloapp?charset=utf8mb4#$MYSQL_URI#g" \
